proc make_beam_particles {e0 e_spread n} {
    global match beam_seed
    set sum_e 0.0
    set f [open particles.tmp w]

    RandomReset -seed $beam_seed -stream User
...
proc make_beam_many {name nslice n} {
    ...
    set ch {1.0}
    calc wake.dat $charge -3 3 $match(sigma_z) $nslice
...
proc make_beam_many_phase {name nslice n dph} {
    global charge e_initial match wakedir beamdir
    set ch {1.0}
    calc $wakedir/wake.dat $charge -3 3 $match(sigma_z) $nslice
    InjectorBeam $name -bunches 1 \
	    -macroparticles [expr $n] \
	    -energyspread 0.0 \
	    -ecut 3.0 \
	    -e0 $e_initial \
	    -file $wakedir/wake.dat \
	    -chargelist $ch \
	    -charge 1.0 \
	    -phase $dph \
	    -overlapp [expr -390*0.3/1.3] \
    ...
    SetRfGradientSingle $name 0 $l    
    make_beam_particles $e_initial $match(e_spread) [expr $nslice*$n]
    BeamRead -file $beamdir/particles.in -beam $name
}

proc make_beam_slice {name nslice n} {
    global charge e_initial match n_total wakedir beamdir
    set ch {1.0}
    calc $wakedir/wake.dat $charge -3 3 $match(sigma_z) $nslice
    InjectorBeam $name -bunches 1 \
	    -macroparticles [expr $n] \
...
proc make_beam_train {name nb nslice n} {
    global charge e_initial match lambda wakedir beamdir
    set ch {1.0}
    calc $wakedir/wake.dat $charge -3 3 $match(sigma_z) $nslice
...
proc make_beam_train_main {name nb nslice n} {
    global charge e_initial match wk_scale wakedir beamdir
    set ch {1.0}
    calc $wakedir/wake.dat $charge -3 3 $match(sigma_z) $nslice
    set wl {}
    lappend wl 0.0
    if {$nb>=2} {
	lappend wl [expr 1000*$wk_scale]
    }
...
proc make_beam_slice35 {name nslice n} {
    global charge e_initial match wakedir beamdir
    set ch {1.0}
    calc $wakedir/wake.dat $charge -3.5 3.5 $match(sigma_z) $nslice
    InjectorBeam $name -bunches 1 \
	    -macroparticles [expr $n] \
	    -energyspread [expr 0.01*$match(e_spread)*$e_initial] \
	    -ecut 3.5 \
